...
class TestPackageXmlFormattedCLI(unittest.TestCase):
    ...
    def test_cli_ignore_formatting_errors_forces_check_only(self):
        """--ignore-formatting-errors should imply check-only and pass the flag through."""
        fake_cls, constructed = make_fake_validator_factory()
        with mock.patch.object(SUT, "PackageXmlValidator", fake_cls):
            with mock.patch.object(
                sys, "argv", ["prog", "--ignore-formatting-errors", str(self.tmpdir)]
            ):
                with mock.patch.dict(os.environ, {"ROS_DISTRO": "jazzy"}, clear=True):
                    buf = io.StringIO()
                    with redirect_stdout(buf):
        ...
        self.assertEqual(len(constructed), 1)
        inst = constructed[0]
        self.assertTrue(inst.init_kwargs["ignore_formatting_errors"])
        self.assertTrue(inst.init_kwargs["check_only"])
        self.assertEqual(inst.check_and_format_calls, [[str(self.tmpdir)]])
