...
class TestManifestSchemaStep(unittest.TestCase):
    ...
    def test_valid_manifest_passes(self):
        result = self.step.perform_check(_parse(VALID), "/tmp/demo_pkg/package.xml")
    ...
    def test_format_2_warns_but_passes(self):
        ...
        result = self.step.perform_check(_parse(xml), "/tmp/demo_pkg/package.xml")
        self.assertTrue(result.valid)
...
"""
    ...
    def test_skipped_under_missing_deps_only(self):
        ...
        result = step.perform_check(_parse(broken), "/tmp/demo_pkg/package.xml")
        self.assertTrue(result.valid)
        self.assertEqual(result.errors, [])
